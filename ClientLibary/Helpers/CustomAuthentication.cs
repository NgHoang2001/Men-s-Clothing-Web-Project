...
namespace ClientLibary.Helpers
{
    public class CustomAuthentication
    {
        ...
        public async Task<ClaimsPrincipal> GetAuthenticationAsync()
        {
            // Lấy token có trong local của user
            var stringToken = LocalStorageFieldService.GetToken();
            if (stringToken == null) { return await Task.FromResult(anonymous); }

            // Chuyển đối string token -> UserSession (token, refreshtoken)
            var deserializeToken = Serializations.DeserializeJsonString<UserSession>(stringToken);
            if (deserializeToken == null) return await Task.FromResult(anonymous);

            // Lấy thông tin có trong token của người dùng
            var getUserClaims = DecryptToken(deserializeToken.Token!);
            if (getUserClaims == null) return await Task.FromResult(anonymous);

            // Thiết lập thông tin xác thức vào ứng dụng
            var claimPrincipal = SetClaimPrincial(getUserClaims);
            if (claimPrincipal == null) return await Task.FromResult(anonymous);
        ...
        public async Task<ClaimsPrincipal> UpdateAuthenticationState(UserSession userSession)
        {
            var claimPrincipal = new ClaimsPrincipal();
            if (userSession.Token != null || userSession.RefreshToken != null)
            {
                var serializeSession = Serializations.SerializeObj(userSession);
                LocalStorageFieldService.SetToken(serializeSession);
                var token = LocalStorageFieldService.GetToken();
                var getUserClaims = DecryptToken(userSession.Token!);
                claimPrincipal = SetClaimPrincial(getUserClaims);
            }
            else
            {
                LocalStorageFieldService.RemoveToken();
            }
            return await Task.FromResult(claimPrincipal);
        }
        ...
        private static CustomUserClaims DecryptToken(string jwtToken)
        {
            ...
            return new CustomUserClaims(Id: userId!.Value, Email: email!.Value, Name: "", Role: role!.Value);
        }
